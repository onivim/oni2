...
class TscTaskProvider extends dispose_1.Disposable {
    ...
    async getAllTsConfigs(token) {
        const configs = arrays_1.flatten(await Promise.all([
            this.getTsConfigForActiveFile(token),
            this.getTsConfigsInWorkspace(token),
        ]));
        return Promise.all(configs.map(async (config) => await fs_1.exists(config.uri) ? config : undefined)).then(arrays_1.coalesce);
    }
    ...
    async getTsConfigsInWorkspace(token) {
        ...
        return Promise.race([
            ...
            testUtils_1.wait(this.findConfigFilesTimeout).then(() => {
    ...
    static async getLocalTscAtPath(folderPath) {
        ...
        if (platform === 'win32' && await fs_1.exists(vscode.Uri.file(path.join(bin, 'tsc.cmd')))) {
            return path.join(bin, 'tsc.cmd');
        }
        else if ((platform === 'linux' || platform === 'darwin') && await fs_1.exists(vscode.Uri.file(path.join(bin, 'tsc')))) {

...
async function acceptFirstSuggestion(uri, _disposables) {
    return testUtils_1.retryUntilDocumentChanges(uri, { retries: 10, timeout: 0 }, _disposables, async () => {
        await vscode.commands.executeCommand('editor.action.triggerSuggest');
        await testUtils_1.wait(1000);
...
async function typeCommitCharacter(uri, character, _disposables) {
    const didChangeDocument = testUtils_1.onChangedDocument(uri, _disposables);
    await vscode.commands.executeCommand('editor.action.triggerSuggest');
    await testUtils_1.wait(3000); // Give time for suggestions to show

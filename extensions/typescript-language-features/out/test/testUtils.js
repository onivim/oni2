...
async function createTestEditor(uri, ...lines) {
    ...
    await editor.insertSnippet(new vscode.SnippetString(exports.joinLines(...lines)), new vscode.Range(0, 0, 1000, 0));
    return editor;
}
...
function assertEditorContents(editor, expectedDocContent, message) {
    ...
    if (cursorIndex >= 0) {
        ...
        assert.deepEqual({ line: editor.selection.active.line, character: editor.selection.active.line }, { line: expectedCursorPos.line, character: expectedCursorPos.line }, 'Cursor position');
    }
}
    ...
    const result = await Promise.race([
        ...
        (async () => {
            for (let i = 0; i < options.retries; ++i) {
                await exports.wait(options.timeout);
                if (done) {
                    return;
                }
                await exec();

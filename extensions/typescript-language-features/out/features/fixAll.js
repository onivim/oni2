...
let TypeScriptAutoFixProvider = /** @class */ (() => {
    class TypeScriptAutoFixProvider {
        ...
        async getFixAllCodeAction(document, file, diagnostics, token) {
            ...
            const codeAction = new vscode.CodeAction(localize('autoFix.label', 'Auto fix'), TypeScriptAutoFixProvider.kind);
    ...
    TypeScriptAutoFixProvider.kind = vscode.CodeActionKind.SourceFixAll.append('ts');
    TypeScriptAutoFixProvider.metadata = {
        providedCodeActionKinds: [TypeScriptAutoFixProvider.kind]
    };
    return TypeScriptAutoFixProvider;
})();

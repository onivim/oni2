...
let ofLines = (~width=100., ~height=100., lines: array(string)) => {
  ...
  let editor = create(~config, ~buffer=buffer |> EditorBuffer.ofBuffer, ())
...
module DSL = {
  let line = (lineNumber) => BytePosition.{
    byte: ByteIndex.zero,
...
//    });
    test("multiple inline elements test", ({expect, _}) => {
      let (editor, buffer) = ofLines([|"Line 1", "Line 2", "Line 3", "Line 4"|]);

      let editor = editor
      |> Editor.addInlineElement(~uniqueId="test1", ~lineNumber=LineNumber.(zero + 1),
      ~height=100)
      |> Editor.addInlineElement(~uniqueId="test2", ~lineNumber=LineNumber.(zero + 2),
      ~height=100);

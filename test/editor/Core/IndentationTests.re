...
let space2 = IndentationSettings.create(~mode=Spaces, ~size=2, ~tabSize=2, ());
let tab4 = IndentationSettings.create(~mode=Tabs, ~size=4, ~tabSize=4, ());
let space4 = IndentationSettings.create(~mode=Spaces, ~size=4, ~tabSize=4, ());
...
describe("Indentation", ({describe, _}) => {
  describe("getLevel", ({test, _}) => {
    ...
    test("string with 2 tab has indentation level of 2, regardless of setting", ({expect}) => {
        let a1 = Indentation.getLevel(tab2, "\t\ttest string");

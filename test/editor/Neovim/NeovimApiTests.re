...
let withNeovimApi = (f) => {
    ...
    let msgpackTransport = MsgpackTransport.make(~onData=nvim.stdout.onData, ~write=nvim.stdin.write, ());
...
describe("NeovimApi", ({test, _}) => {
  test("nvim__id", ({expect}) => {
    withNeovimApi((api) => {
        let result = api.requestSync("nvim__id", Msgpck.List([Msgpck.String("hey, neovim!")]));
  ...
  test("nvim__id_array", ({expect}) => {
    withNeovimApi((api) => {
        let result = api.requestSync("nvim__id", Msgpck.List([Msgpck.List([Msgpck.String("hey, neovim!"), Msgpck.String("hey again, neovim!")])]));

...
describe("MsgpackTransport", ({test, _}) => {
  ...
  test("publishes multiple messages", ({expect}) => {
    ...
    let firstMessageBytes = Msgpck.String("hello") |> Msgpck.Bytes.to_string;
    let secondMessageBytes =
      Msgpck.List([Msgpck.Int(1), Msgpck.String("Hello World again!")])
      |> Msgpck.Bytes.to_string;
    ...
    | _ =>
      expect.string("fail").toEqual("Failed deserializing first message")
    };
  });
});

...
describe("MenuJob", ({describe, _}) => {
  ...
  let createItem = (name) => {
                         ...
                         category: None,
                         name,
                         command: () => Oni_Model.Actions.ShowNotification(Oni_Model.Notification.create(~title="derp", ~message=name, ())),
                         icon: None
                      };
                      ret;
  ...
  describe("boundary cases", ({test, _}) => {
    test("large amount of items added work", ({expect, _}) => {
      let job = MenuJob.create();
              ...
              let commands: list(Actions.menuCommand) = List.init(1000000, 
              i => createItem("Item " ++ string_of_int(i)));

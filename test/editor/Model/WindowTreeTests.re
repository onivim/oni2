...
describe("WindowTreeTests", ({describe, _}) =>
  describe("addSplit", ({test, _}) => {
    test("add vertical split", ({expect}) => {
      ...
      expect.bool(splits ==Parent(Vertical, [
        Leaf(split)
      ])).toBe(true);

      let split2 = createSplit(~editorGroupId=2, ());

      let splits = addSplit(~target=Some(targetId), Vertical, split2, splits);
      expect.bool(splits ==Parent(Vertical, [
        Leaf(split2),
        Leaf(split),
      ])).toBe(true);
    ...
    test("parent split changes direction if needed", ({expect}) => {
      ...
      let splits = addSplit(~target=Some(targetId), Horizontal, split2, splits);
      ...
      let splits = addSplit(~target=Some(targetId), Vertical, split3, splits);

...
function activate(context) {
    const codeLensProvider = {
        provideCodeLenses: (document, token) => {
            ...
            return [new vscode.CodeLens(range1, command1), new vscode.CodeLens(range2, command2)]
        },
    }
    ...
    const commonFormat = (str) => {
        return [new vscode.TextEdit(new vscode.Range(1, 2, 3, 4), str)]
    }
    ...
    const signatureHelpProvider = {
        provideSignatureHelp: (_document, _position, _token, _context) => {
            const signature1 = new vscode.SignatureInformation(
                "signature 1",
    ...
    const referenceProvider = {
        provideReferences: (document, _position, _token) => {
            return [new vscode.Location(document.uri, new vscode.Range(1, 2, 3, 4))]
        },
    }

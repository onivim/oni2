...
type t = {
  highlights: list(internalHighlight),
}

// Only report highlightgs within one second
let threshold = 1.0;

let _filterActive = (currentTime: Time.t, highlights: list(internalHighlight)) => {
 let filter = (_) => {
  true
 }
...
let _internalToExternal: internalHighlight => highlight = (internalHighlight) => {
  range: internalHighlight.range,
  highlightType: internalHighlight.highlightType,
};

let _externalToInternal: Time.t => highlight => internalHighlight = (time) => (internalHighlight) => {
 time,
 range: internalHighlight.range, 
 highlightType: internalHighlight.highlightType,
};
...
let add = (time, newHighlights, model) => {
  ...
  let newHighlights = newHighlights
  |> List.map(_externalToInternal(time));

...
let current = (state: State.t) => {
  (variable) => {
    ...
    let lineIdx = CharacterPosition.(cursorPosition.line) |> EditorCoreTypes.LineNumber.toZeroBased;
    ...
    | "TM_CURRENT_LINE" =>
      ...
      |> OptionEx.flatMap(buffer => {
        ...
        if (lineIdx >= 0 && lineIdx < lineCount) {
          Some(Buffer.getLine(lineIdx,buffer)
          |> BufferLine.raw)
        } else {
          None
        }
    ...
    | "TM_FILENAME" => maybePath
    |> Option.map(Filename.basename)

    | "TM_FILENAME_BASE" => maybePath
    |> Option.map(Filename.basename)
    |> Option.map(Filename.remove_extension)

    | "TM_DIRECTORY" => maybePath

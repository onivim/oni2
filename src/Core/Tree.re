...
let toList: t('node, 'leaf') => list(listView('node, 'leaf)) =
  tree => {
  ...
  (module
   {
     module Tree = {
       let leaf = v => Leaf(v);
       let node = (~expanded=true, ~children=[], v) =>
         Node({expanded, data: v, children});
     };
     ...
     let%test "single child" = {
       Tree.leaf(1) |> toList == [View.leaf(1)];
     };
     let%test "single parent" = {
       Tree.node(1) |> toList == [View.node(1)];
     };

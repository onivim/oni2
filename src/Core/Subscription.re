...
let create =
    (id: string, provider: provider('action, 'params), params: 'params) =>
...
module Runner = (Config: {
    ...
    let (added, remaining) =
      List.fold_left(
        ((added, remaining), subscription: t(Config.action)) => {
          let Subscription({id, _}) = subscription;

          switch (Hashtbl.find_opt(jobs, id)) {
          | None => ([subscription, ...added], remaining)

          | Some(_) => (added, [subscription, ...remaining])
          };
        },
        ([], []),
        subscriptions,
  ...
  let dispose = (subscription: t(Config.action)) => {
    let Subscription({id, provider: (module Provider), _}) = subscription;
  ...
  let run = (~dispatch, subscriptions) => {
    let (added, removed, remaining) =
      subscriptions |> applyNamespaceToIds |> diff;

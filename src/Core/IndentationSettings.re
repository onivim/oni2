...
let normalizeTabs = (~indentation: t, str: string) => {
  if (indentation.mode == Tabs) {
    ...
    let indentation = List.init(tabCount, (_idx) => indentationSpaces)
    |> String.concat("");

    let stringWithoutTabs = String.sub(str, tabCount, String.length(str) - tabCount);
    indentation ++ stringWithoutTabs
  }
};

let%test_module "normalizeTabs" = (module {

...
type t('kind) = 
| UNC({server: string, share: string, path: Fp.t(absolute)}): t(absolute)
...
let isDescendent = (~ofPath, path) => {
    switch ((ofPath, path)) {
    | (UNC({path: ofPath, _}), UNC({path, _})) => Fp.isDescendent(~ofPath, path)
...
let dirName = fun
| UNC({path,server, share}) => UNC({server, share, path: Fp.dirName(path)})
| Path(path) => Path(Fp.dirName(path))

let eq = (pathA, pathB) => switch ((pathA, pathB)) {
| (UNC({path: pathA, server: serverA, share: shareA}), UNC({path: pathB, server: serverB, share: shareB})) =>
    Fp.eq(pathA, pathB) && String.equal(serverA, serverB) && String.equal(shareA, shareB)
| (Path(pathA), Path(pathB)) => Fp.eq(pathA, pathB)
| _ => false;
};

module At = {
   let (/) = (path, name) => switch(path) {
   | UNC({path, server, share}) => UNC({server, share, path: Fp.At.(path / name)})
   | Path(path) => Path(Fp.At.(path / name))
   } 
}

let hasParentDir = fun

...
let make = (~model, ~theme, ~font, ()) => {
  let renderItem =
      (extensions: array(Exthost.Extension.Scanner.ScanResult.t), idx) => {
    ...
    <View
      ...
      <View style=Style.[flexDirection(`Column), flexGrow(1)]>
        <Text
          ...
          text={Exthost.Extension.Manifest.getDisplayName(extension.manifest)}
        />
  ...
  let bundledExtensions =
    Extensions.getExtensions(
      ~category=Exthost.Extension.Scanner.Bundled,
      model,
    );

  let userExtensions =
    Extensions.getExtensions(~category=Exthost.Extension.Scanner.User, model);

  ...
  (module
   {
       ...
       (module
        {
          let%test "Returns range when anchor comes first" = {
            create(~anchor=3, ~focus=5) |> length == 2;
          };

          let%test "Returns range when anchor comes last" = {
            create(~anchor=5, ~focus=3) |> length == 2;
          };

          let%test "Returns 0 for collapsed selection" = {
            create(~anchor=3, ~focus=3) |> length == 0;
          };
        });
       ...
       (module
        {
          let%test "Returns anchor" = {
            create(~anchor=3, ~focus=5) |> offsetLeft == 3;
          };

          let%test "Returns focus" = {
            create(~anchor=5, ~focus=3) |> offsetLeft == 3;
          };
          let%test "Returns any" = {
            create(~anchor=3, ~focus=3) |> offsetLeft == 3;
          };
        });
     let%test_module "offsetRight" =
       (module
        {
          let%test "Returns anchor" = {
            create(~anchor=5, ~focus=3) |> offsetRight == 5;
          };

          let%test "Returns focus" = {
            create(~anchor=3, ~focus=5) |> offsetRight == 5;
          };

          let%test "Returns any" = {
            create(~anchor=3, ~focus=3) |> offsetRight == 3;
          };
        });
       ...
       (module
        {
          let%test "Returns true" = {
            create(~anchor=3, ~focus=3) |> isCollapsed == true;
          };
          let%test "Returns false" = {
            create(~anchor=3, ~focus=7) |> isCollapsed == false;
          };
        });

...
let update = (extHostClient, model: t, msg) => {
  ...
  | Started({id, cmd}) =>
    let idToTerminal =
      IntMap.add(id, {id, cmd, rows: 80, columns: 40, pid: None, title: None}, model.idToTerminal);
    let lastId = max(id, model.lastId);
    ({idToTerminal, lastId}, Isolinear.Effect.none);
  | Service(ProcessStarted({id, pid})) => 
    let idToTerminal = IntMap.update(id, Option.map(term => { ...term, pid: Some(pid)}), model.idToTerminal);
    ({ ...model, idToTerminal}, Isolinear.Effect.none);
  | Service(ProcessTitleSet({id, title})) => 
    let idToTerminal = IntMap.update(id, Option.map(term => { ...term, title: Some(title)}), model.idToTerminal);
    ({ ...model, idToTerminal}, Isolinear.Effect.none);

...
let filesystem = (
	...
	~rename: (~source: Uri.t, ~target: Uri.t, Files.FileOverwriteOptions.t) => Lwt.t(unit),
	~copy: (~source: Uri.t, ~target: Uri.t, Files.FileOverwriteOptions.t) => Lwt.t(unit),
	...
	let mapEncoder = (encoder) => v => {
		v
		|> Json.Encode.encode_value(encoder)
		|> Reply.okJson
	};
	...
	Msg.(
	switch (msg) {
	| FileSystem(fileSystemMsg) => switch(fileSystemMsg) {
		| Stat({uri}) => stat(uri) |> Lwt.map(mapEncoder(Files.StatResult.encode));
		| ReadDir({uri}) => readdir(uri) |> Lwt.map(mapEmpty)
		// TODO: How to serialize buffer?
		| ReadFile({uri}) => readFile(uri) |> Lwt.map(mapEmpty)
		| WriteFile({uri, buffer}) => writeFile(uri, buffer) |> Lwt.map(mapEmpty)
		| Rename({source, target, opts}) =>
		rename(~source, ~target, opts) |> Lwt.map(mapEmpty);
		| Copy({source, target, opts}) => copy(~source, ~target, opts) |> Lwt.map(mapEmpty);

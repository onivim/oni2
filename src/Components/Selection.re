...
let collapseRelative =
    (~text: string, ~selection: t, direction: directionVariant): t => {
  switch (direction) {
  | Left(offset) => collapse(~text, rangeStart(selection) - abs(offset))
  | Right(offset) => collapse(~text, rangeEnd(selection) + abs(offset))
  };
};

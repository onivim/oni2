...
module VisualRange = {
  ...
  [@deriving show({with_path: false})]
    ...
    let create = (~startLine, ~startColumn, ~endLine, ~endColumn, ~mode, ()) => {
        let (startLine, startColumn, endLine, endColumn) = _normalizeRange(startLine, startColumn, endLine, endColumn);
...
module AutoCommandContext = {
  ...
  let create = (~activeBufferId, ~cursorLine, ~cursorColumn, ~modified, ~visualRange, ()) => {
    activeBufferId,
    cursorLine,
    cursorColumn,
    modified,

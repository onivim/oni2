...
let showPendingWork = (~contextPrinter=(_) => "n/a", v: pendingWork('context)) => {
  "- Pending Work\n"
  ++ " -- latest buffer version: " ++ string_of_int(v.version)
  ++ " -- ranges left to process: "
  ++ string_of_int(List.length(v.remainingRanges))
  ++ " -- context: " ++ contextPrinter(v.context);
};

let showCompletedWork = (~workPrinter=(_) => "n/a", v: completedWork('v)) => {
  List.fold_left((prev, curr) => {
    let (key, info) = curr;
    prev ++ "\n" ++ "Line " ++ string_of_int(key) ++ ": " ++ " version " ++ string_of_int(info.version) ++ " - work: " ++ workPrinter(info.v);
...
let doWork = (f, p: pendingWork('context), c: completedWork('v)) => {
  ...
  | [outerHd, ...outerTail] => {
      ...
      | [hd, ...tail] => {
        ...
        let lines = IntMap.update(line, (prev) => {
          switch (prev) {
          | Some(v) => 

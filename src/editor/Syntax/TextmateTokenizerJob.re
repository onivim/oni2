...
let doWork = (pending: pendingWork, completed: completedWork) => {
 ...
 | Some(grammar) => {
    ...
    let scopes = switch(IntMap.find_opt(currentLine - 1, completed)) {
    | None => Grammar.getScopeStack(grammar)
    | Some(v) => v.scopeStack;
    };

    // Get new tokens & scopes
    let (tokens, scopes) = Grammar.tokenize(
    ...
    |> List.map((token) => {
      open Token;
        ...
        |> List.filter((s) => s != pending.scope);
        (token.position, scopes)
      })
    |> List.filter((scopes) => switch(scopes) {
    | (_, []) => false
    | _ => true
    });
    
    let tokens = List.map((token) => {
    ...
    let completed = IntMap.update(currentLine, (prev) => switch(prev) {

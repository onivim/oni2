...
let rec getTokens = (
    ...
    let ret: list(t) = switch (tokens) {
    | [] => _getAllTokens(s, defaultForegroundColor, pos, String.length(s))
    | [last] => _getAllTokens(s, ColorMap.get(colorMap, last.foregroundColor, defaultForegroundColor, defaultBackgroundColor), last.index, String.length(s))
    | [v1, v2, ...tail] => {
        let nextBatch = _getAllTokens(s, ColorMap.get(colorMap, v1.foregroundColor, defaultForegroundColor, defaultBackgroundColor), v1.index, v2.index);
        List.append(nextBatch, getTokens([v2, ...tail], v2.index, s, theme, colorMap));
    }
    };
...
let tokenize:
  ...
  (s, theme, tokenColors, colorMap) => {
      getTokens(tokenColors, 0, s, theme, colorMap)
          |> List.rev;
  };

...
let getMaxKey = (map: IntMap.t(int)) => {
    let (maxKey, _) = IntMap.fold((key, v, curr) => {
...
let guessIndentation = (~f: (int) => string, lineCount: int, defaultTabSize: int, defaultInsertSpaces: bool) => {
    ...
    while (i^ < linesToCheck) {
        ...
        if (foundChar) {
            ...
            let (prevSpaceCount, _, prevFoundChar) = getLeadingWhitespace(prevLine);
            if (prevFoundChar) {
                let diff = abs(prevSpaceCount - spaceCount);
                spaceDelta := IntMap.update(diff, (curr) => switch(curr) {
                | Some(v) => Some(v + 1);
                | None => Some(1);
                }, spaceDelta^);
            }
    ...
    let shouldInsertSpaces = if (linesWithSpaces == linesWithTabs) {
        defaultInsertSpaces
    ...
    let size = switch (shouldInsertSpaces) {
    | false => defaultTabSize
    | true => getMaxKey(spaceDelta^);
    }

    let mode = shouldInsertSpaces ? IndentationSettings.Spaces : IndentationSettings.Tabs;
    {mode, size };
};

let guessIndentationArray = (lines: array(string), defaultTabSize: int, defaultInsertSpaces: bool) => {
    let f = (i) => Array.get(lines, i);
    guessIndentation(~f, Array.length(lines), defaultTabSize, defaultInsertSpaces);
}

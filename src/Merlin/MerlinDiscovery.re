...
let show = (v: t) => {
  let omp =
    switch (v.ocamlMerlinPath) {
    | Some(v) => v
    | None => "None"
    };

  let omrp =
    switch (v.ocamlMerlinReasonPath) {
    | Some(v) => v
    | None => "None"
    };
...
let discover = (workingDirectory: string) => {
  ...
  let ret =
    ...
    | None =>
      ...
      // Try esy first
      let (ocamlMerlinPath, ocamlMerlinReasonPath) =
        ...
        | Some(v) =>
          ...
          | Ok(v) when v == Esy.Status.ReadyForDev =>
            ...
            | (Ok(mp), Ok(mrp)) => (Some(mp), Some(mrp))
            | (Ok(mp), Error(_)) => (Some(mp), None)
            | (Error(e), _) =>
      ...
      let (ocamlMerlinPath, ocamlMerlinReasonPath) =
        switch (ocamlMerlinPath, ocamlMerlinReasonPath) {
        | (Some(mp), Some(rmp)) => (Some(mp), Some(rmp))
        | (Some(mp), None) => (
            Some(mp),
            Environment.which(merlinReasonExecutable),
          )
        | (None, Some(rmp)) => (
            Environment.which(merlinExecutable),
            Some(rmp),
          )
        | (None, None) => (
            Environment.which(merlinExecutable),
      ...
      let result = {ocamlMerlinPath, ocamlMerlinReasonPath};

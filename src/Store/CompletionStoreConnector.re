...
module Effects = {
  let requestCompletions =
    ...
    Isolinear.Effect.createWithDispatch(~name="requestCompletions", dispatch => {
      prerr_endline ("CompletionStoreConnector.requestCompletions  - requesting....");
      ...
      Lwt.on_success(completionPromise, completions => {
        prerr_endline ("CompletionStoreConnector.requestCompletions  - Got: " ++ string_of_int(List.length(completions)) ++ " completoins.");

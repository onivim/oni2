...
let start = () => {
    ...
    Isolinear.Effect.createWithDispatch(~name="completion.checkMeet", (dispatch) => {
      ...
      | Some(ed) => {
        ...
        | Some(buffer) => {
          ...
          let meetOpt = Model.CompletionMeet.getMeetFromBufferCursor(~cursor=ed.cursorPosition, buffer);
          switch (meetOpt) {
          | None => (); 
          | Some(meet) =>
        ...
        if (!equals(~line, ~column, ~bufferId, lastMeet^)) {
          ...
          Log.info("[Completion] New completion meet: " ++ Model.CompletionMeet.show(meetOpt));
          lastMeet := newMeet;
          dispatch(Actions.CompletionStart(newMeet));
        } else {
          // If we're at the same position... but our base is different...
          // fire a base change
          if (!String.equal(meet.base, lastBase^)) {
            lastBase := meet.base;
  ...
  let updater = (state: Model.State.t, action: Actions.t) => {
    ...
    | Actions.ChangeMode(mode) when mode == Vim.Types.Insert =>
      (state, checkCompletionMeet(state))
    | Actions.CursorMove(position) =>
      (state, checkCompletionMeet(state))
    | Actions.BufferUpdate(buf) =>

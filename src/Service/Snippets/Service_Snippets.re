...
module Decode = {
  ...
  let snippetLines = list(string)
  |> and_then(lines => {
    let parseResult = lines
    |> String.concat("\n")
    |> Snippet.parse;
  ...
  let decode = Json.Decode.(
    ...
    |> map(List.map(((description, prefixAndBody: PrefixAndBody.t)) => {
      let prefix = prefixAndBody.prefix
...
module Cache = {
  let fileToPromise: Hashtbl.t(string, Lwt.t(list(SnippetWithMetadata.t))) = Hashtbl.create(16);

  let get = (filePath: string) => {
    switch (Hashtbl.find_opt(fileToPromise, filePath)) {
    | Some(promise) => promise
    | None =>
      let promise = Lwt.bind(Service_OS.Api.readFile(filePath), bytes => {
        let json = bytes
        |> Bytes.to_string

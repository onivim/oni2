...
runTest(~configuration=Some(configuration), ~name="VimExperimentalVimlTest", ({wait, runEffects, input, staysTrue, _}) => {
  ...
  staysTrue(~timeout=5.0, ~name="additional notifications do not show up", (state: State.t) => {
    let notifications = Feature_Notification.all(state.notifications);

    let helloNotifications = notifications
    |> List.filter(notification => Feature_Notification.(notification.message == "hello-world"))

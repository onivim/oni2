...
runTest(~configuration, ~name="LanguageReasonMerlinDiagnosticsTest", (dispatch, wait, runEffects) => {
  ...
  wait(~timeout=30., ~name="Wait for active buffer to be set", (state: State.t) => {
  ...
  wait(~timeout=30., ~name="Wait for diagnostic to show up", (state: State.t) => {
  ...
  wait(~timeout=30., ~name="Wait for diagnostic to be removed", (state: State.t) => {
    let buffer = Buffers.getBuffer(id, state.buffers);
    switch (buffer) {
    | None => false
    | Some(v) => 

...
f_add(typval_T *argvars, typval_T *rettv)
{
  ...
  {
    if ((l = argvars[0].vval.v_list) != NULL && !var_check_lock(l->lv_lock, (char_u *)N_("add() argument"), TRUE) && list_append_tv(l, &argvars[1]) == OK)
      copy_tv(&argvars[0], rettv);
  }
  else if (argvars[0].v_type == VAR_BLOB)
  {
    if ((b = argvars[0].vval.v_blob) != NULL && !var_check_lock(b->bv_lock,
...
{
  rettv->vval.v_number = tv_get_number_chk(&argvars[0], NULL) & tv_get_number_chk(&argvars[1], NULL);
}
...
f_argc(typval_T *argvars, typval_T *rettv)
{
  ...
  else if (argvars[0].v_type == VAR_NUMBER && tv_get_number(&argvars[0]) == -1)
...
f_argv(typval_T *argvars, typval_T *rettv)
{
  ...
  if (argvars[0].v_type != VAR_UNKNOWN)
  {
    ...
    else if (argvars[1].v_type == VAR_NUMBER && tv_get_number(&argvars[1]) == -1)
    {
      arglist = GARGLIST;
      argcount = GARGCOUNT;
    }
...
find_buffer(typval_T *avar)
{
  ...
  else if (avar->v_type == VAR_STRING && avar->vval.v_string != NULL)
  {
    ...
    if (buf == NULL)
    {
      ...
      FOR_ALL_BUFFERS(buf)
      if (buf->b_fname != NULL && (path_with_url(buf->b_fname)
#ifdef FEAT_QUICKFIX
                                   || bt_nofile(buf)
#endif
                                       ) &&
...
f_bufnr(typval_T *argvars, typval_T *rettv)
{
  ...
  /* If the buffer isn't found and the second argument is not zero create a
     * new buffer. */
  if (buf == NULL && argvars[1].v_type != VAR_UNKNOWN && tv_get_number_chk(&argvars[1], &error) != 0 && !error && (name = tv_get_string_chk(&argvars[0])) != NULL && !error)
...
f_deletebufline(typval_T *argvars, typval_T *rettv)
{
  ...
  if (buf->b_ml.ml_mfp == NULL || first < 1 || first > buf->b_ml.ml_line_count || last < first)
  {
    rettv->vval.v_number = 1; /* FAIL */
    return;
  }
...
f_empty(typval_T *argvars, typval_T *rettv)
{
  ...
  case VAR_BLOB:
    n = argvars[0].vval.v_blob == NULL || argvars[0].vval.v_blob->bv_ga.ga_len == 0;
    break;
...
f_expand(typval_T *argvars, typval_T *rettv)
{
  ...
  rettv->v_type = VAR_STRING;
  if (argvars[1].v_type != VAR_UNKNOWN && argvars[2].v_type != VAR_UNKNOWN && tv_get_number_chk(&argvars[2], &error) && !error)
  ...
  else
  {
    ...
    if (argvars[1].v_type != VAR_UNKNOWN && tv_get_number_chk(&argvars[1], &error))
...
f_extend(typval_T *argvars, typval_T *rettv)
{
  ...
  if (argvars[0].v_type == VAR_LIST && argvars[1].v_type == VAR_LIST)
  {
    ...
    if (l1 != NULL && !var_check_lock(l1->lv_lock, arg_errmsg, TRUE) && l2 != NULL)
    {
      if (argvars[2].v_type != VAR_UNKNOWN)
      {
  ...
  else if (argvars[0].v_type == VAR_DICT && argvars[1].v_type == VAR_DICT)
  {
    ...
    if (d1 != NULL && !var_check_lock(d1->dv_lock, arg_errmsg, TRUE) && d2 != NULL)
    {
...
f_foldtext(typval_T *argvars UNUSED, typval_T *rettv)
{
  ...
  if (foldstart > 0 && foldend <= curbuf->b_ml.ml_line_count && dashes != NULL)
  {
    ...
    r = alloc(STRLEN(txt) + STRLEN(dashes) // for %s
              + 20                         // for %3ld
...
get_buffer_info(buf_T *buf)
{
  ...
  dict_add_number(dict, "lnum", buf == curbuf ? curwin->w_cursor.lnum : buflist_findlnum(buf));
...
f_win_execute(typval_T *argvars, typval_T *rettv)
{
  ...
  if (wp != NULL)
  {
    if (switch_win_noblock(&save_curwin, &save_curtab, wp, curtab, TRUE) == OK)
    {
      check_cursor();
  ...
  if (argvars[1].v_type != VAR_UNKNOWN)
  {
    ...
    if (argvars[2].v_type != VAR_UNKNOWN)
    {
      if (tv_get_number_chk(&argvars[2], &error))
        rettv_list_set(rettv, NULL);
      if (argvars[3].v_type != VAR_UNKNOWN && tv_get_number_chk(&argvars[3], &error))
  ...
  if (argvars[2].v_type != VAR_UNKNOWN)
  {
    ...
    if (argvars[3].v_type != VAR_UNKNOWN)
    {
      if (tv_get_number_chk(&argvars[3], &error))
        rettv_list_set(rettv, NULL);
      if (argvars[4].v_type != VAR_UNKNOWN && tv_get_number_chk(&argvars[4], &error))
        flags |= WILD_ALLLINKS;
    }
  }
  if (file != NULL && !error)
...
f_insert(typval_T *argvars, typval_T *rettv)
{
  ...
  else if ((l = argvars[0].vval.v_list) != NULL && !var_check_lock(l->lv_lock,
...
{
  rettv->vval.v_number = argvars[0].v_type == VAR_FLOAT && isnan(argvars[0].vval.v_float);
}
#endif
...
find_some_match(typval_T *argvars, typval_T *rettv, matchtype_T type)
{
  ...
  if (type == MATCH_LIST || type == MATCH_POS)
  {
    ...
    if (rettv_list_alloc(rettv) == FAIL)
      goto theend;
    if (type == MATCH_POS && (list_append_string(rettv->vval.v_list,
                                                 (char_u *)"", 0) == FAIL ||
                              list_append_number(rettv->vval.v_list,
                                                 (varnumber_T)-1) == FAIL ||
                              list_append_number(rettv->vval.v_list,
...
f_matchadd(typval_T *argvars UNUSED, typval_T *rettv UNUSED)
{
  ...
  if (argvars[2].v_type != VAR_UNKNOWN)
  {
    ...
    if (argvars[3].v_type != VAR_UNKNOWN)
    {
      ...
      if (argvars[4].v_type != VAR_UNKNOWN && matchadd_dict_arg(&argvars[4], &conceal_char, &win) == FAIL)
        return;
    }
  }
  if (error == TRUE)
...
f_remove(typval_T *argvars, typval_T *rettv)
{
  ...
  if (argvars[0].v_type == VAR_DICT)
  {
    ...
    else if ((d = argvars[0].vval.v_dict) != NULL && !var_check_lock(d->dv_lock, arg_errmsg, TRUE))
    {
  ...
  else if ((l = argvars[0].vval.v_list) != NULL && !var_check_lock(l->lv_lock, arg_errmsg, TRUE))
  {
    idx = (long)tv_get_number_chk(&argvars[1], &error);
    if (error)
...
f_resolve(typval_T *argvars, typval_T *rettv)
{
...
#ifdef HAVE_READLINK
  {
    ...
    if (!vim_ispathsep(*p))
    {
      if (is_relative_to_current && *p != NUL && !(p[0] == '.' && (p[1] == NUL || vim_ispathsep(p[1]) || (p[1] == '.' && (p[2] == NUL || vim_ispathsep(p[2]))))))
      {
        /* Prepend "./". */
...
f_setenv(typval_T *argvars, typval_T *rettv UNUSED)
{
  ...
  if (argvars[1].v_type == VAR_SPECIAL && argvars[1].vval.v_number == VVAL_NULL)
    vim_unsetenv(name);
  else
...
do_sort_uniq(typval_T *argvars, typval_T *rettv, int sort)
{
  ...
  else
  {
    ...
    if (sort)
    {
      ...
      else
      {
        ...
        qsort((void *)ptrs, (size_t)len, sizeof(sortItem_T),
              info.item_compare_func == NULL && info.item_compare_partial == NULL
                  ? item_compare
                  : item_compare2);
    ...
    else
    {
      ...
      item_compare_func_ptr = info.item_compare_func != NULL || info.item_compare_partial != NULL
                                  ? item_compare2
                                  : item_compare;
...
f_swapname(typval_T *argvars, typval_T *rettv)
{
  ...
  rettv->v_type = VAR_STRING;
  buf = tv_get_buf(&argvars[0], FALSE);
  if (buf == NULL || buf->b_ml.ml_mfp == NULL || buf->b_ml.ml_mfp->mf_fname == NULL)
    rettv->vval.v_string = NULL;
  else
